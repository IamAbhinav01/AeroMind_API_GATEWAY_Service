...
const { BUCKET_CAPACITY, BUCKET_REFILL, BUCKET_TIMEOUT, REDIS_TIMEOUT } = require('../config/server.config');
const TOKEN_BUCKET_SCRIPT = require('./lua_script');
...
class RedisTokenBucketService {
    ...
    async getBucketStatus(identifier) {
        ...
        try {
            ...
            const refillRateMs    = this.defaultRefill / this.defaultTimeout;
            ...
            return { exists: false, identifier, error: error.message };
        }
    }

    async isRequestAllowed(identifier, options = {}) {
        const capacity = options.capacity ?? this.defaultCapacity;
        const refill   = options.refill   ?? this.defaultRefill;
        ...
        try {
            const result = await this.redisClient.eval(
                ...
                capacity, refillRateMs, Date.now(), cost
            );
            return {
                allowed: result[0] === 1,
            ...
            console.error('[TokenBucket] Redis error:', error.message);
    ...
    async resetBucket(identifier) {
        ...
        try {
            ...
            return { success: false, identifier, error: error.message };
        }
    }
    ...
    async listAllBuckets() {
        ...
        try {
            ...
            return { count: 0, buckets: [], error: error.message };
        }
    }
}
